...
// Props tipi tanımlandı
interface ProductBannerProps {
  itemsPerPageFromBanner: (itemsPerPage: number) => void; // `number` alan bir fonksiyon
}
  ...
  return (
    <div className="w-full flex flex-col md:flex-row md:items-center justify-end">
      <div className="flex items-center gap-2 md:gap-6 mt-4 md:mt-0">
        ...
        {/* Ürün sayısı seçme alanı */}
        <div className="flex items-center gap-2 text-[#767676] relative">
          <label className="block">Show:</label>
          <select
            onChange={(e) => itemsPerPageFromBanner(Number(e.target.value))} // `+` yerine `Number()` kullanıldı
            id="items-per-page"
